...
def train(player_X):
    ...
    while True:
        ...
        if state.player_to_move == 1:
            ...
            state_old = agent.get_state(state,-1)
            previous_point = evaluate(state,0,-1)
            move = agent.get_action(state_old, state)
            if move != -1:
                final_move = np.zeros(81)
                final_move[move] = 1
                new_move = translate_move(move,-1)
                is_valid = state.is_valid_move(new_move)
                # print (new_move)
        ...
        state.act_move(new_move)
        if state.player_to_move == 1:
            reward = evaluate(state,0,-1) - previous_point
            state_new = agent.get_state(state, -1)
            agent.train_short_memory(state_old, final_move, reward, state_new, is_game_done or state.game_over)
            agent.remember(state_old, final_move, reward, state_new, is_game_done or state.game_over)
        turn += 1
...
def play_fight(player_X):
    ...
    for i in range (10):
        ...
        while True:
            ...
            if state.game_over or is_game_done:
                if(state.game_result(state.global_cells.reshape(3,3))==1): no_win +=1
